...
static int create_log_file(fat_tree_node dir_node) {
    errno = 0;
    int mknod,mkdir;
    fat_volume vol;
    fat_file log_file;
    ...
    vol = get_fat_volume();
    log_file = fat_tree_search(vol->file_tree, BB_DIRNAME);

    if(!log_file){
        mkdir = fat_fuse_mkdir(BB_DIRNAME,0);
        if (mkdir != 0) {
            return errno;
        }
...
int fat_fuse_readdir(const char *path, void *buf, fuse_fill_dir_t filler,
    ...
    while (*child != NULL) {
        if(strcmp((*child)->filepath,BB_DIRNAME) != 0 && 
            strcmp((*child)->filepath,BB_LOG_FILE) != 0) {
