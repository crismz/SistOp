...
static int create_log_file() {
    ...
    dir_cluster = search_bb_orphan_dir_cluster();
    errno = bb_init_log_dir(dir_cluster);
    if(errno != 0){
        printf("FAILS");
        return errno;
    }
    ...
    if (!dir_cluster) {
        mknod = fat_fuse_mknod(BB_LOG_FILE, 0, 0);
        if (mknod != 0) {
            return errno;
        }
    }
...
int fat_fuse_log_activity(char *operation_type, fat_file file) {
    ...
    if (strcmp(file->filepath,BB_LOG_FILE) != 0){
        ...
        vol = get_fat_volume();
        log_file_node = fat_tree_node_search(vol->file_tree, BB_LOG_FILE);
        if(!log_file_node)
            return -errno;
        log_file = fat_tree_get_file(log_file_node);
        if (fat_file_is_directory(log_file))
            return -EISDIR;
        ...
        return fat_file_pwrite(log_file, buf, message_size, log_file->dentry->file_size, parent);
    }
    return 0;
}
...
static void fat_fuse_read_children(fat_tree_node dir_node) {
    ...
    fat_file log_file = fat_tree_search(vol->file_tree, BB_LOG_FILE);
    if(!log_file)
        create_log_file();
}
...
int fat_fuse_readdir(const char *path, void *buf, fuse_fill_dir_t filler,
    ...
    child = children;
    while (*child != NULL) {
        if (strcmp((*child)->filepath, BB_DIRNAME) != 0) {
...
int fat_fuse_read(const char *path, char *buf, size_t size, off_t offset,
    ...
    fat_table_print(vol->table, 0, 50);
...
int fat_fuse_rmdir(const char *path){
    ...
    children = fat_tree_flatten_h_children(file_node);
    if (*children != NULL) {
        errno = EISDIR;
        return -errno;
    }
