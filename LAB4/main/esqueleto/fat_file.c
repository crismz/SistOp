...
void fat_file_delete(fat_file file, fat_file parent) {
    u32 start_cluster = 0, next_cluster = 0;

    start_cluster = file->start_cluster;

    next_cluster = fat_table_get_next_cluster(file->table, start_cluster);
    // Mark as free
    
    fat_table_set_next_cluster(file->table, start_cluster,
                               FAT_CLUSTER_FREE);
    ...
    // Mark following clusters as not used
    while (fat_table_cluster_is_valid(start_cluster)) {
        // If there was an error, we continue with the function.
        next_cluster = fat_table_get_next_cluster(file->table, start_cluster);
        fat_table_set_next_cluster(file->table, start_cluster, FAT_CLUSTER_FREE);
        start_cluster = next_cluster;
    }
    // Update entrance in directory
